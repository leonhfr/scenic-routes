...
You will need to get Flickr and Mapbox API keys and save them to the `.env` file.

You need to have a redis database running.

If you run the app for the first time, you will need to build the database and the static GeoJSON files. Run:
```shell
npm run build-db
```

Then, to run the back-end:
```shell
npm start
```
